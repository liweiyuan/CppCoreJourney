...
public:
    ...
    static double SecToUs(double sec) {
        return sec * 1000000;
    }

    static double UsToSec(double us) {
        return us / 1000000;
    }

    static double MsToUs(double ms) {
        return ms * 1000;
    }

    static double UsToMs(double us) {
        return us / 1000;
    }
};
...
TEST(TestsChrono, test_chrono_convert) {
    ...
    EXPECT_EQ(std::chrono::duration_cast<std::chrono::milliseconds>(s).count(), 1000);
    EXPECT_EQ(std::chrono::duration_cast<std::chrono::seconds>(ms).count(), 1);
    EXPECT_EQ(std::chrono::duration_cast<std::chrono::microseconds>(us).count(), 1000000);
}
